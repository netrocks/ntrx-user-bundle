...
class UserAuthenticator extends AbstractFormLoginAuthenticator implements PasswordAuthenticatedInterface
{
	...
	public function __construct(EntityManagerInterface $entityManager, UrlGeneratorInterface $urlGenerator, CsrfTokenManagerInterface $csrfTokenManager, UserPasswordEncoderInterface $passwordEncoder, ParameterBagInterface $params) {
		...
		$this->params = $params;
		$this->loginRedirect = $params->get('ntrx_user.login_redirect');
	...
	public function getUser($credentials, UserProviderInterface $userProvider) {
		...
		$user = $this->entityManager->getRepository($this->userClass)->findOneBy(['email' => $credentials['email']]);
	...
	public function onAuthenticationSuccess(Request $request, TokenInterface $token, $providerKey) {
		...
		return new RedirectResponse($this->urlGenerator->generate($this->loginRedirect));
	}
